//Cops and Robbers by Bar Harel
#include <a_samp>
#undef MAX_PLAYERS
#define MAX_PLAYERS (30)
#define MAX_HOUSES (65)
#define MAX_HOUSE_MODELS (6)
#define HOUSE_LIMIT (2)
#define AllowPicks(%0) SetTimerEx("AllowPick", 1000, false, "i", %0)
#define DisablePicks(%0) pInfo[%0][disablepick] = true
#define SCMR(%0) SendClientMessage(playerid, Col_Red, %0)
#define SCMG(%0) SendClientMessage(playerid, Col_Green, %0)
#include <core>
#include <float>
...
#define UserPath "Users/%s.ini" //Will define user's account path. In this case, we will save it in Scriptfiles/Users. So create a folder inside of your Scriptfiles folder called Users
...
enum PlayerInfo
{
	...
	Name[MAX_PLAYER_NAME], //User's name
	...
	HouseAmount, //How many houses does the player own?
	...
	/*Hitman*/
	Hitter, //Who hit him
	Hitting, //Who did the /hit
	HitAmount, //Amount
	//HitTill, //When should hit end
	/*Vehicles*/
...
enum MovingObjects
{
	...
	gGamEraserID,
	bool:gGamEraserState //True for open, False for closed.
}
enum Menus
{
	Menu:mWeapons,
...
{
	print("\n---------------------------------------");
	print("Running New Cops and Robbers - by Bar Harel\n");
	print("---------------------------------------\n");
}
...
{
    new str[128];
    format(str,sizeof(str),UserPath,pInfo[playerid][Name]);
    return str;
}
stock hPath(houseid)
{
...
{
    new str[128];
    format(str,sizeof(str),BanPathL,pInfo[playerid][Name]);
    return str;
}
stock IPBanPath(IP[])
{
...
stock jailplayer(playerid, arresterid){
	new formatted[128], wantedl = GetPlayerWantedLevel(playerid);
	...
	format(formatted, 128, "Wanted criminal %s(%d) has been jailed by %s(%d) for %d seconds.", pInfo[playerid][Name], playerid, pInfo[arresterid][Name], arresterid, pInfo[playerid][JailTime]);
...
stock BanPlayer(banning, target, reason[], btime = -1){
	new pip[16], formatted[128], pname2[MAX_PLAYER_NAME];
	if (banning != -1) GetPlayerName(banning,pname2,MAX_PLAYER_NAME);
	else pname2 = "Autoban";
	GetPlayerIp(target, pip, 16);
	if(fexist(WhitePath(pInfo[target][Name]))){
		fremove(WhitePath(pInfo[target][Name]));
		if(banning != -1) SendClientMessage(banning, Col_Green, "Player was whitelisted. Whitelist removed.");
	}
	if(btime == -1) {
		format(formatted, 128, "%s has been permanently banned for %s.", pInfo[target][Name], reason);
	...
	format(formatted, 128, "Player: %s", pInfo[target][Name]);
...
stock HousePicks(playerid, pickupid)
{
	...
	if (!strcmp(hOwnerName[houseid], pInfo[playerid][Name], true)){
		...
		format(formatted, 128, "This house belongs to %s.", hOwnerName[houseid]);
...
stock LoadObjects()
{
	...
	MovingObject[gGamEraserID] = CreateDynamicObject(17951, 1520.66174, -694.68091, 95.57840,   0.00000, 0.00000, 182.86290);
	/* */
	return 1;
}
...
stock LoadOwnedVehicle(playerid, modelid, Float:ToX, Float:ToY, Float:ToZ){
	...
	if(pInfo[playerid][vModel1] == modelid){
		...
		if (vInfo[tempvID][OwnGroup] == gPlayer && !strcmp(vOwnerName[tempvID], pInfo[playerid][Name]) && GetVehicleModel(tempvID) == modelid) {
	...
	strpack(vOwnerName[tempvID], pInfo[playerid][Name]);
...
stock PoliceRadioStore(playerid, storename[]){
	new formatted[128];
	format(formatted, 128, "***Police Radio*** Please be advised, %s(%d) is robbing %s.", pInfo[playerid][Name], playerid, storename);
...
public OnPlayerConnect(playerid)
{
	...
	pInfo[playerid][Name] = pname;
	GetPlayerIp(playerid, pip, 16);
	if (fexist(BanPath(playerid)) && !fexist(WhitePath(pname)) && strcmp("[GCnR]GamEraser", pname) != 0){
	...
	}else if (fexist(IPBanPath(pip)) && !fexist(WhitePath(pname)) && strcmp("[GCnR]GamEraser", pname) != 0){
    ...
    {//then we will 'force' him to register :)
		if (!strcmp(pname, "[GCnR]", true, 6)  && strcmp("[GCnR]GamEraser", pname) != 0){
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	if (dialogid == dclasses)
	{
		switch (listitem){
			...
			case 3:
			{
				pInfo[playerid][Class] = cHitman;
				SendClientMessage(playerid, Col_Green, "You are now a hitman. Your job is to kill players in the /hits list.");
				SendClientMessage(playerid, Col_Green, "DO NOT kill innocent players, even if they have hits.");
			}
		}
    ...
    {//then
        ...
        {//then
			...
			INI_WriteInt(file, "SpawnHouse1",-1);
			...
			pInfo[playerid][sHits] = 0;
			pInfo[playerid][SpawnHouse] = -1;
			pInfo[playerid][HouseAmount] = 0;
			pInfo[playerid][DailyLogin] = getdate();
			pInfo[playerid][sTimeSpent] = 0;
			pInfo[playerid][LoginTime] = gettime();
			format(formatted, 128, "%s (id: %d) has connected.", pInfo[playerid][Name], playerid);
    ...
    {//then
        ...
        {//then
            ...
            {//then
				...
				if (pInfo[playerid][Adminlevel] >= 4) pInfo[playerid][adseepm] = true;
            }
	...
	else if(dialogid == dcmds) // cmds
	{
		if(!response) return 1;
			...
			strcat(string, "\n/drugs\t\tCall for a drug dealer\n/me\t\tWrite an emotion.\n/hit\t\tPut a hit contract on a player's head.");
			...
			case cHitman:
				ShowPlayerDialog(playerid, dcmdslist, DIALOG_STYLE_MSGBOX, "Hitman commands", "/hits\t\tSee a list of open hits.", "OK", "");
			default:
	...
	else if(dialogid == drules) // rules
	{
		...
		strcat(stringy, "\n{292929}Black player{FFFFFF}\n\tHitman.\n\tGot a hit? This guy is here to kill you. Treat them as innocent unless they shoot you.");
	...
	else if (dialogid == dhospital)
	{
		...
		if (!hInfo[pInfo[playerid][CurrentHouse]][owned]){
			switch (listitem){
				case 0:
				{
					if (pInfo[playerid][BankMoney] < hInfo[pInfo[playerid][CurrentHouse]][hPrice]) SendClientMessage(playerid, Col_Red, "You do not have enough money in your bank to buy this house.");
					else if (pInfo[playerid][HouseAmount] >= HOUSE_LIMIT && GetPVarInt(playerid, "Admin") < 4) SendClientMessage(playerid, Col_Red, "You already own 2 houses.");
					else{
						if (pInfo[playerid][HouseAmount] >= HOUSE_LIMIT && GetPVarInt(playerid, "Admin") >= 4) SendClientMessage(playerid, Col_Red, "WARNING: You already own 2 houses or more. Please do not abuse this.");
						GetPlayerName(playerid, hOwnerName[pInfo[playerid][CurrentHouse]], MAX_PLAYER_NAME);
						...
						SetPlayerPos(playerid, hInfo[pInfo[playerid][CurrentHouse]][hExitX], hInfo[pInfo[playerid][CurrentHouse]][hExitY], hInfo[pInfo[playerid][CurrentHouse]][hExitZ]);
						pInfo[playerid][HouseAmount]++;
						pInfo[playerid][CurrentHouse] = -1;
					}
				}
				case 1: EnterHouse(playerid);
			}
		}else if (!strcmp (hOwnerName[pInfo[playerid][CurrentHouse]], pInfo[playerid][Name], true)){
			switch (listitem){
		...
		if(!response) return 1;
		new amount, formatted[148], total;
		if (sscanf(inputtext, "i", amount)) SendClientMessage(playerid, Col_Red, "Please specify an amount.");
			...
			format(formatted, 148, "You have bought an amount of %d drugs from %s. Total price: $%d.", amount, pInfo[pInfo[playerid][DealerID]][Name], total);
			SendClientMessage(playerid, Col_Pink, formatted);
			format(formatted, 148, "%s has bought an amount of %d drugs. Total price: $%d. You received a bonus of 2 score points.", pInfo[playerid][Name], amount, total);
		...
		format(formatted, 148, "%s(%d) is selling you drugs for $%d per gram for a max of %d grams.\n Please type the amount of drugs below.", pInfo[pInfo[playerid][DealerID]][Name], pInfo[playerid][DealerID], pInfo[playerid][DealingPrice], pInfo[pInfo[playerid][DealerID]][Drugs]);
...
public OnPlayerSpawn(playerid)
{
	...
	if(CITY_LOS_SANTOS == gPlayerCitySelection[playerid]) {
		...
		else if (pInfo[playerid][SpawnHouse] != -1 && (hInfo[pInfo[playerid][SpawnHouse]][hExitX] != -1 && hInfo[pInfo[playerid][SpawnHouse]][hExitY] != -1 && hInfo[pInfo[playerid][SpawnHouse]][hExitZ] != -1)) SetPlayerPos(playerid, hInfo[pInfo[playerid][SpawnHouse]][hExitX], hInfo[pInfo[playerid][SpawnHouse]][hExitY], hInfo[pInfo[playerid][SpawnHouse]][hExitZ]);
		else{
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	if (PRESSEDKEY(KEY_YES)){ //(Y)
		...
		if (IsPlayerInRangeOfPoint(playerid, 10.0, 1520.66174, -694.68091, 95.57840) && !strcmp(pInfo[playerid][Name], "[GCnR]GamEraser", true)){
			if (!MovingObject[gGamEraserState]){
				MoveDynamicObject(MovingObject[gGamEraserID], 1520.66174, -694.68091, 91.97930, 2.0);
				MovingObject[gGamEraserState] = true;
				SendClientMessage(playerid, Col_Green, "Garage Opening.");
			}else{
				MoveDynamicObject(MovingObject[gGamEraserID], 1520.66174, -694.68091, 95.57840, 2.0);
				MovingObject[gGamEraserState] = false;
				SendClientMessage(playerid, Col_Green, "Garage Closing.");
			}
			return 1;
		}
...
{ 
	if(killerid == INVALID_PLAYER_ID && pInfo[playerid][raped]){
		new formatted[128];
		format(formatted, 128, "%s(%d) has died from a sex disease.", pInfo[playerid][Name], playerid);
		SendClientMessageToAll(Col_LightOrange, formatted);
		SendDeathMessage(killerid, playerid, reason);
	}else if (pInfo[killerid][Class] == cCop && GetPlayerWantedLevel(playerid) > 1){
		new formatted[128];
		format(formatted, 128, "Wanted criminal %s(%d) has been killed by officer %s(%d).", pInfo[playerid][Name], playerid, pInfo[killerid][Name], killerid);
		SendClientMessageToAll(Col_LightOrange, formatted);
		SetPlayerScore(killerid, GetPlayerScore(killerid)+1);
		if (GetPlayerWantedLevel(playerid) < 10) SendClientMessage(playerid, Col_Pink, "You have killed a wanted criminal and recieved a 1500$ bonus +  1 score!"), pInfo[killerid][Money] += 1500;
		else SendClientMessage(playerid, Col_Pink, "You have killed a most wanted criminal and recieved a 3000$ bonus + 1 score!"), pInfo[killerid][Money] += 3000;
		SendDeathMessage(killerid, playerid, reason);
	}else if(pInfo[killerid][Class] == cHitman){
		if (pInfo[playerid][Hitter] != -1 && (GetPlayerWantedLevel(playerid) > 0 || pInfo[playerid][Class] == cCop)){
			new formatted[128];
			pInfo[killerid][Money] += pInfo[playerid][HitAmount];
			format(formatted, 128, "%s(%d) has been killed by a hitman.", pInfo[playerid][Name], playerid);
			SendClientMessageToAll(Col_LightOrange, formatted);
			SendClientMessage(pInfo[playerid][Hitter], Col_Pink, "Another soul is down in hell. Pleasure doing buisness with you.");
			pInfo[playerid][Hitter] = -1;
			format(formatted, 128, "Good job lad, you have earned $%d + 3 score points.", pInfo[playerid][HitAmount]);
			SendClientMessage(killerid, Col_Pink, formatted);
			SetPlayerScore(killerid, GetPlayerScore(killerid) + 3);
			pInfo[killerid][sHits] += 1;
			CommitedCrime(killerid, 10);
		} else if (pInfo[playerid][Hitter] != -1){
			SendDeathMessage(killerid, playerid, reason);
			new stringy[40] = "deathmatching. Please read /rules";
			CallRemoteFunction("adjailplayer", "iis[40]", killerid,  180, stringy);
			SendClientMessage(killerid, Col_Red, "You are not allowed to kill an innocent player even if he has a hit.");
			SendDeathMessage(killerid, playerid, reason);
		} else if (GetPlayerWantedLevel(killerid) > 0 || GetPlayerWantedLevel(playerid) > 0) SendDeathMessage(killerid, playerid, reason);
		else {
			SendDeathMessage(killerid, playerid, reason);
			new stringy[40] = "deathmatching. Please read /rules";
			CallRemoteFunction("adjailplayer", "iis[40]", killerid,  180, stringy);
			SendClientMessage(killerid, Col_Red, "You are not allowed to kill an innocent player.");
			SendDeathMessage(killerid, playerid, reason);
		}
	}else if (GetPlayerWantedLevel(killerid) == 0 && GetPlayerWantedLevel(playerid) == 0 && ((pInfo[killerid][Class] == cCop && pInfo[killerid][Hitter] == -1) || pInfo[killerid][Class] != cCop)){
			SendDeathMessage(killerid, playerid, reason);
			new stringy[40] = "deathmatching. Please read /rules";
			CallRemoteFunction("adjailplayer", "iis[40]", killerid,  180, stringy);
			if (pInfo[killerid][Class] == cCop) SendClientMessage(killerid, Col_Red, "You are not allowed to kill a hitman if you don't have a hit.");
			else SendClientMessage(killerid, Col_Red, "You are not allowed to kill a hitman if you're innocent.");
			SendDeathMessage(killerid, playerid, reason);
	}else{
		SendDeathMessage(killerid, playerid, reason); //So hitmen won't show in kill log
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
	...
	if (newstate == PLAYER_STATE_PASSENGER){
		...
		if (vInfo[pInfo[playerid][CurrentVeh]][OwnGroup] == gAdmin) SendClientMessage(playerid, Col_Blue, "You are driving an admin vehicle. Please make sure not to abuse it. Use it only for admin work.");
			...
			if (strcmp(vOwnerName[pInfo[playerid][CurrentVeh]], pInfo[playerid][Name]) == 0) SendClientMessage(playerid, Col_LightOrange, "This is your vehicle. Only you can drive it.");
			else {
...
{
	new whisper[128], formatted[128], Float:pX, Float:pY, Float:pZ;
	...
	GetPlayerPos(playerid, pX, pY, pZ);
	format(formatted, 128, "%s(%d) whispers: %s", pInfo[playerid][Name], playerid, whisper);
...
COMMAND:cw(playerid, params[])
{
	...
	format(formatted, 128, "%s(%d) car whispers: %s", pInfo[playerid][Name], playerid, whisper);
	foreach (new i : Player)
	{
...
COMMAND:kill(playerid)
{
	...
	format(formatted, 128, "%s(%d) has commited suicide using /kill.", pInfo[playerid][Name], playerid);
	SendClientMessageToAll(Col_Red, formatted);
	return 1;
}
COMMAND:cuff(playerid, params[])
{
	...
	format(formatted, 128, "You have been cuffed by %s. You may try to break them using /bc. Your cuffs will be automatically removed within 20 seconds.", pInfo[playerid][Name]);
	SendClientMessage(target, Col_Pink, formatted);
	format(formatted, 128, "You have cuffed %s. Please make sure you do not leave him cuffed.", pInfo[target][Name]);
	SendClientMessage(playerid, Col_Pink, formatted);
	...
	if (pInfo[playerid][Class] == cWeaponDeal){
		...
		foreach(new i : Player){
			if (pInfo[i][needweapons]){
				format(formatted, 128, "%s(%d)", pInfo[i][Name], i);
...
COMMAND:uncuff(playerid, params[])
{
	...
	format(formatted, 128, "You have been uncuffed by %s.", pInfo[playerid][Name]);
	SendClientMessage(target, Col_Pink, formatted);
	format(formatted, 128, "You have uncuffed %s.", pInfo[target][Name]);
	SendClientMessage(playerid, Col_Pink, formatted);
...
{
	new pm[128], formatted[128], target;
	...
	format(formatted, 128, "PM from %s(%d): %s", pInfo[playerid][Name], playerid, pm);
	SendClientMessage(target, Col_Yellow, formatted);
	format(formatted, 128, "PM to %s(%d): %s", pInfo[target][Name], target, pm);
	SendClientMessage(playerid, Col_Yellow, formatted);
	pInfo[target][LastPM] = playerid;
	format(formatted, 128, "%s(%d) -> %s(%d): %s", pInfo[playerid][Name], playerid, pInfo[target][Name], target, pm);
...
COMMAND:offer(playerid, params[])
{
	if (pInfo[playerid][Class] == cWeaponDeal){
		...
		if (pInfo[target][needweapons]){
			...
			new formatted[128];
			format(formatted, 128, "%s has offered you to buy weapons. /weapons to agree.", pInfo[playerid][Name]);
			SendClientMessage(target, Col_Pink, formatted);
		}
		return 1;
	}else if (pInfo[playerid][Class] == cDrugDeal){
		...
		new target, price, Float:pX, Float:pY, Float:pZ, formatted[148];
		...
		if (pInfo[target][needdrugs]){
			...
			format(formatted, 148, "%s(%d) is selling you drugs for $%d per gram for a max of %d grams.\n Please type the amount of drugs below.", pInfo[playerid][Name], playerid, price, pInfo[playerid][Drugs]);
			...
			format(formatted, 148, "%s has offered you to buy drugs for $%d per gram. /drugs to agree.", pInfo[playerid][Name], price);
			SendClientMessage(target, Col_Pink, formatted);
		}
		return 1;
	}
...
COMMAND:weapons(playerid)
{
	...
	new dealers, formatted[128];
	format(formatted, 128, "%s(%d) has called for a weapon dealer.", pInfo[playerid][Name], playerid);
	foreach (new i : Player)
	{
...
COMMAND:drugs(playerid)
{
	...
	new dealers, formatted[128];
	format(formatted, 128, "%s(%d) has called for a drug dealer.", pInfo[playerid][Name], playerid);
	foreach (new i : Player)
	{
...
COMMAND:gc(playerid, params[])
{
	...
	format(formatted, 128, "You have given %s %d$.", pInfo[target][Name], amount);
	SendClientMessage(playerid, Col_Pink, formatted);
	pInfo[playerid][Money] -= amount;
	format(formatted, 128, "%s has given you %d$.", pInfo[playerid][Name], amount);
	SendClientMessage(target, Col_Pink, formatted);
...
COMMAND:r(playerid, params[])
{
	...
	format(formatted, 128, "Reply to %s(%d): %s", pInfo[pInfo[playerid][LastPM]][Name], pInfo[playerid][LastPM], pm);
	SendClientMessage(playerid, Col_Yellow, formatted);
	format(formatted, 128, "%s(%d) -> %s(%d): %s", pInfo[playerid][Name], playerid, pInfo[pInfo[playerid][LastPM]][Name], pInfo[playerid][LastPM], pm);
	SendPMToAdmins(formatted);
...
COMMAND:lock(playerid)
{
	...
	if (vInfo[pInfo[playerid][CurrentVeh]][OwnGroup] == gPlayer){
		if (strcmp(pInfo[playerid][Name], vOwnerName[pInfo[playerid][CurrentVeh]], true) != 0) return SendClientMessage(playerid, Col_Red, "You cannot lock a vehicle which isn't yours.");
	}
	GetVehicleParamsEx(pInfo[playerid][CurrentVeh], engine, lights, alarm, doors, bonnet, boot, objective);
...
COMMAND:escape(playerid)
{
	...
	if(random(10)<7) {
		...
		new formatted[128];
		format(formatted, 128, "%s has failed to escape from jail. Jail time extended.", pInfo[playerid][Name]);
		SendClientMessageToAll(Col_LightOrange, formatted);
...
COMMAND:rape(playerid, params[])
{
	...
	if (!pInfo[target][alive]){
		format(formatted, 128, "You have raped %s's corpse.", pInfo[target][Name], target);
		SendClientMessage(playerid, Col_Pink, formatted);
		format(formatted, 128, "%s(%d) has raped your corpse. eeeew!", pInfo[playerid][Name], playerid);
		SendClientMessage(target, Col_Pink, formatted);
		return 1;
	}
	...
	CommitedCrime(playerid, 3);
	format(formatted, 128, "You have raped %s(%d) and infected him with a sex disease.", pInfo[target][Name], target);
	SendClientMessage(playerid, Col_Pink, formatted);
	format(formatted, 128, "You have been raped by %s(%d) and infected with a sex disease. Run to the hospital to recover!", pInfo[playerid][Name], playerid);
...
COMMAND:rob(playerid, params[])
{
	...
	new formatted[128], robamount = random(pInfo[target][Money]);
	if (pInfo[target][WalletsLeft] > 0){
		...
		format(formatted, 128, "%s(%d) has tried robbing you, but you had a wallet. You now have %d wallets left.", pInfo[playerid][Name], playerid, pInfo[target][WalletsLeft]);
		SendClientMessage(target, Col_Pink, formatted);
		return 1;
	}
	format(formatted, 128, "Robbed by %s(%d)~n~%d$", pInfo[playerid][Name], playerid, robamount);
	GameTextForPlayer(target, formatted, 1500, 6);
	format(formatted, 128, "Robbed by %s(%d) for %d$.", pInfo[playerid][Name], playerid, robamount);
	SendClientMessage(target, Col_Pink, formatted);
	pInfo[playerid][Money] += robamount;
	format(formatted, 128, "Robbed %s(%d)~n~%d$", pInfo[target][Name], target, robamount);
	GameTextForPlayer(playerid, formatted, 1500, 6);
	format(formatted, 128, "Robbed %s(%d) for %d$.", pInfo[target][Name], target, robamount);
	SendClientMessage(playerid, Col_Pink, formatted);
...
{
	if (pInfo[playerid][Class] == cHitman) return SendClientMessage(playerid, Col_Red, "Hitmans cannot create hits.");
	new target, amount;
	if (sscanf(params, "ii", target, amount)) return SendClientMessage(playerid, Col_Red, "USAGE: /hit <id> <amount>");
	if (!IsPlayerConnected(target)) return SendClientMessage(playerid, Col_Red, "Wrong ID: No such player.");
	if (target == playerid) return SendClientMessage(playerid, Col_Red, "Wish to suicide? Jump off a building...");
	if (amount < 50000) return SendClientMessage(playerid, Col_Red, "Can't you afford at least 50,000 for his head?");
	if (pInfo[playerid][Money] < amount) return SendClientMessage(playerid, Col_Red, "You do not have enough cash homie");
	//if (pInfo[target][HitTill] < gettime()) pInfo[target][Hitter] = -1;
	if (pInfo[target][Hitter] != -1) return SendClientMessage(playerid, Col_Red, "The person already has a hit");
	new formatted[128];
	pInfo[target][Hitter] = playerid;
	pInfo[playerid][Hitting] = target;
	pInfo[target][HitAmount] = amount;
	pInfo[playerid][Money] -= amount;
	//pInfo[target][HitTill] = gettime() + 600;
	format(formatted, 128, "%s(%d) has just received a hit by %s(%d) for $%d. Time for someone to die.", target, pInfo[target][Name], playerid, pInfo[playerid][Name], amount);
	foreach(new i : Player) if(pInfo[i][Class] == cHitman) SendClientMessage(i, Col_Black, formatted);
	SendClientMessage(playerid, Col_Pink, "You have just made a deal with the devil.");
	SendClientMessage(target, Col_Pink, "Watch it. Someone wants you dead...");
	return 1;
}
COMMAND:hits(playerid)
{
	if (pInfo[playerid][Class] != cHitman) return SendClientMessage(playerid, Col_Red, "You are not a hitman.");
	new formatted[128], bool:listy;
	SendClientMessage(playerid, Col_Black, "Hit list:");
	foreach(new i : Player) {
		if (pInfo[i][Hitter] != -1){
			listy=true;
			format(formatted, 128, "%s(%d) placed by %s(%d) for $%d.", pInfo[i][Name], i, pInfo[i][Hitter], pInfo[pInfo[i][Hitter]][Name], pInfo[i][HitAmount]);
			SendClientMessage(playerid, Col_Black, formatted);
		}
	}
	if (!listy) SendClientMessage(playerid, Col_Black, "There are no open hits.");
	else SendClientMessage(playerid, Col_Black, "Kill them.");
	return 1;	
}
COMMAND:cm(playerid, params[])
{
	...
	new formatted[128], msg[101];
	if (sscanf(params, "s[128]", msg)) return SendClientMessage(playerid, Col_Red, "USAGE: /cm [message]");
	format(formatted, 128, "*CM* %s(%d): {FFFFFF}%s", pInfo[playerid][Name], playerid, msg);
...
COMMAND:adresetspawn(playerid, params[])
{
	if (GetPVarInt(playerid, "Admin") < 4) return 0;
	new target;
	if (sscanf(params, "i", target)) return SendClientMessage(playerid, Col_Red, "USAGE: /adresetspawn [playerid]");
	if (!IsPlayerConnected(target)) return SendClientMessage(playerid, Col_Red, "Wrong ID: No such player.");
	pInfo[target][SpawnHouse] = -1;
	SendClientMessage(playerid, Col_Green, "Player's spawn has been reset.");
	SendClientMessage(target, Col_Green, "Your spawn location has been reset by an admin.");
	return 1;
}
COMMAND:adipban(playerid, params[])
{
	if (GetPVarInt(playerid, "Admin") < 4) return 0;
	new target, formatted[128],playerIP[18];
	if (sscanf(params, "i", target)) return SendClientMessage(playerid, Col_Red, "USAGE: /adipban [playerid]");
	if (!IsPlayerConnected(target)) return SendClientMessage(playerid, Col_Red, "Wrong ID: No such player.");
	if (!strcmp("[GCnR]GamEraser", pInfo[target][Name])) return SendClientMessage(playerid, Col_Red, "You cannot ban the owner.");
	GetPlayerIp(target, playerIP, sizeof(playerIP));
	format(formatted, 128, "banip %s", playerIP);
	SendRconCommand(formatted);
	Kick(target);
	SendClientMessage(playerid, Col_Green, "IP banned.");
	return 1;
}
COMMAND:adipdban(playerid, params[])
{
	if (GetPVarInt(playerid, "Admin") < 5) return 0;
	new target, formatted[128], playerIP[18];
	if (sscanf(params, "i", target)) return SendClientMessage(playerid, Col_Red, "USAGE: /adipdban [playerid]");
	if (!IsPlayerConnected(target)) return SendClientMessage(playerid, Col_Red, "Wrong ID: No such player.");
	if (!strcmp("[GCnR]GamEraser", pInfo[target][Name])) return SendClientMessage(playerid, Col_Red, "You cannot ban the owner.");
	GetPlayerIp(target, playerIP, sizeof(playerIP));
	fremove(Path(target));
	...
	SendRconCommand(formatted);
	Kick(target);
	SendClientMessage(playerid, Col_Green, "IP banned, account deleted.");
	return 1;
}
...
{
	if (GetPVarInt(playerid, "Admin") < 4) return 0;
	new target, pIP[16], formatted[128];
	...
	format(formatted, 128, "Name: %s", pInfo[target][Name]);
	SendClientMessage(playerid, Col_Pink, formatted);
	...
	SendClientMessage(playerid, Col_Pink, formatted);
	if (pInfo[target][Class] == cCop) formatted = "Cop";
	else if (pInfo[target][Class] == cHitman) formatted = "Hitman";
	else if (pInfo[target][Class] == cDrugDeal) formatted = "Drug Dealer";
	else if (pInfo[target][Class] == cRobber) formatted = "Normal Robber";
	else if (pInfo[target][Class] == cNone) formatted = "None (dead)";
	else if (pInfo[target][Class] == cWeaponDeal) formatted = "Weapon Dealer";
	else formatted = "CLASS NOT ADDED, CONTACT DEVELOPERS";
	format(formatted, 128, "Class: %s", formatted);
	SendClientMessage(playerid, Col_Pink, formatted);
...
COMMAND:taze(playerid, params[])
{
	...
	format(formatted, 128, "You have been tazed by %s(%d).", pInfo[playerid][Name], playerid);
	SendClientMessage(target, Col_LightOrange, formatted);
	format(formatted, 128, "You have tazed %s(%d).", pInfo[target][Name], target);
	SendClientMessage(playerid, Col_LightOrange, formatted);
	TogglePlayerControllable(target, 0);
	SetTimerEx("restoretaze", 1250, false, "i", target);
	return 1;
}
...
COMMAND:search(playerid, params[])
{
	...
	if (IsPlayerInRangeOfPlayer(playerid, target, 5.0) == 0) return SendClientMessage(playerid, Col_Red, "You are too far away.");
	if (pInfo[playerid][CurrentVeh] != 0) return SendClientMessage(playerid, Col_Red, "Please exit your vehicle first.");
...
COMMAND:color(playerid, params[])
{
	...
	new color1, color2;
	if (vInfo[pInfo[playerid][CurrentVeh]][OwnGroup] != gPlayer || strcmp(vOwnerName[pInfo[playerid][CurrentVeh]], pInfo[playerid][Name], true)) return SendClientMessage(playerid, Col_Red, "This is not your private vehicle.");
...
COMMAND:adspawnv(playerid, params[]) //Vehicle Spawner
{
	...
	if(sscanf(params, "s[32]D(0)D(0)", Vehicle, ColorOne, ColorTwo)) return SendClientMessage(playerid, Col_Red, "USAGE: /adspawnv [Vehiclename/Vehicleid] [Color 1 (optional)] [Color 2 (optional)]");
	...
	if (vInfo[pInfo[playerid][Spawnedv]][OwnGroup] == gAdmin && !strcmp(pInfo[playerid][Name], vOwnerName[pInfo[playerid][Spawnedv]])) DestroyVehicle(pInfo[playerid][Spawnedv]);
	...
	strpack(vOwnerName[pInfo[playerid][Spawnedv]], pInfo[playerid][Name]);
...
COMMAND:gotoh(playerid, params[])
{
	if (GetPVarInt(playerid, "Admin") < 4) return 0;
	new target;
	if (sscanf(params, "i", target)) return SendClientMessage(playerid, Col_Red, "USAGE: /goto [house ID]");
	if (!fexist(hPath(target))) return SendClientMessage(playerid, Col_Red, "Wrong ID: No such house.");
	SGPlayerPos(playerid, 1);
	if(IsPlayerInAnyVehicle(playerid) == 1){
		SetVehiclePos(pInfo[playerid][CurrentVeh], hInfo[target][hExitX], hInfo[target][hExitY], hInfo[target][hExitZ]);
		SetVehicleVirtualWorld(pInfo[playerid][CurrentVeh], 0);
		SetPlayerVirtualWorld(playerid, 0);
	}else{
		SetPlayerInterior(playerid, 0);
		SetPlayerVirtualWorld(playerid, 0);
		SetPlayerPos(playerid, hInfo[target][hExitX], hInfo[target][hExitY], hInfo[target][hExitZ]);
	}
	return 1;
}
...
{
	new formatted[268], formatted1[40];
	format(formatted1, 40, "%s's stats:", pInfo[playerid][Name]);
	pInfo[playerid][sTimeSpent] += gettime() - pInfo[playerid][LoginTime];
	pInfo[playerid][LoginTime] = gettime();
	format(formatted, 268, "Arrests:\t\t%d\nRobs:\t\t%d\nRapes:\t\t%d\nWeapon Deals:\t%d\nDrug Deals:\t%d\n", pInfo[playerid][sArrests], pInfo[playerid][sRobs], pInfo[playerid][sRapes], pInfo[playerid][sWeaponDeals], pInfo[playerid][sDrugDeals]);
	format(formatted, 268, "%sHits:\t\t%d\nTime Spent:\t%d Days %d Hours %d Minutes", formatted, pInfo[playerid][sHits], floatround((pInfo[playerid][sTimeSpent]/86400),floatround_floor), floatround((pInfo[playerid][sTimeSpent]%86400)/3600,floatround_floor), floatround((pInfo[playerid][sTimeSpent]%3600)/60,floatround_floor));
...
COMMAND:deleteh(playerid, params[])
{
	...
	if (hInfo[tempID][owned]) format(formatted, 128, "Warning: House was owned by %s.", hOwnerName[tempID]), SendClientMessage(playerid, Col_Orange, formatted);
	if(IsValidDynamicPickup(hInfo[tempID][hPickup])) DestroyDynamicPickup(hInfo[tempID][hPickup]);
...
COMMAND:adresethcount(playerid, params[]){
	if(GetPVarInt(playerid, "Admin") < 4) return 0;
	new target, count, formatted[128];
	if (sscanf(params, "i", target)) return SCMR("USAGE: /adresethcount [target]");
	if(!IsPlayerConnected(target)) return SCMR("Wrong ID: No such player.");
	SCMG("House count:");
	for(new i; i!=MAX_HOUSES; i++){
		if (!strcmp(pInfo[target][Name], hOwnerName[i], true)){
			count++;
			format(formatted, 128, "House ID: %d", i);
			SCMG(formatted);
		}
	}
	pInfo[target][HouseAmount] = count;
	SCMG("Total Houses:");
	valstr(formatted, count);
	SCMG(formatted);
	return 1;
}
...
public OnPlayerText(playerid, text[]){
    new formatted[128];
    format(formatted, 128, "%s(%d): {FFFFFF}%s", pInfo[playerid][Name], playerid, text);
...
public OnPlayerDisconnect(playerid, reason)
{
	...
	if (pInfo[playerid][raped]) KillTimer(pInfo[playerid][RapeTimer]), pInfo[playerid][raped] = false;
	...
	if (pInfo[playerid][login]){ //To avoid saving unlogged user's data
		...
		INI_WriteInt(file, "SpawnHouse1",pInfo[playerid][SpawnHouse]);
		INI_WriteInt(file, "HouseAmount",pInfo[playerid][HouseAmount]);
		...
		INI_WriteInt(file, "Hits",pInfo[playerid][sHits]);
		INI_WriteInt(file, "DrugDeal",pInfo[playerid][sDrugDeals]);
		INI_WriteInt(file, "TimeSpent",pInfo[playerid][sTimeSpent]);
		INI_Close(file);//Now after we've done saving their data, we now need to close the file
		switch(reason)
		{
			case 0: format(formatted, 128, "%s has left the server. (Lost Connection)", pInfo[playerid][Name]);
			case 1: format(formatted, 128, "%s has left the server. (Leaving)", pInfo[playerid][Name]);
			case 2: format(formatted, 128, "%s has left the server. (Kicked)", pInfo[playerid][Name]);
		} 
		SendClientMessageToAll(Col_Gray, formatted);
	...
	if (pInfo[playerid][Hitter] != -1 && pInfo[pInfo[playerid][Hitter]][Hitting] == playerid) SendClientMessage(pInfo[playerid][Hitter], Col_Pink, "Your hit target has disconnected, half of the money was returned to you."), pInfo[pInfo[playerid][Hitter]][Money] += (pInfo[playerid][HitAmount]/2);
	if (pInfo[playerid][Hitting] != -1 && playerid == pInfo[pInfo[playerid][Hitting]][Hitter]) pInfo[pInfo[playerid][Hitting]][Hitter] = -1, SendClientMessage(pInfo[playerid][Hitting], Col_Pink, "Hit cancelled due to player leaving.");
	pInfo[playerid][Hitter] = -1;
	pInfo[playerid][Hitting] = -1;
    return 1;
}
public RobbingStore(playerid, pickupid){
	...
	SendClientMessage(playerid, Col_Pink, formatted);
	format(formatted, 128, "%s has robbed %s for $%d.", pInfo[playerid][Name], StoreName, amount);
...
public jailupdate(playerid){
	...
	if (pInfo[playerid][JailTime]>1) {
		...
		pInfo[playerid][JailTime]=0;
		format(formatted, 128, "%s(%d) has been released from jail.", pInfo[playerid][Name], playerid);
		SendClientMessageToAll(Col_LightOrange, formatted);
		SetPlayerInterior(playerid, 6);
		...
		pInfo[playerid][JailTime]=0;
		format(formatted, 128, "%s(%d) has escaped from jail!", pInfo[playerid][Name], playerid);
		SendClientMessageToAll(Col_LightOrange, formatted);
		SetPlayerPos(playerid, 264.0510,82.0633,1001.0391);
...
public loadaccount_user(playerid, name[], value[])
{
	...
	INI_Int("SpawnHouse1",pInfo[playerid][SpawnHouse]);
	INI_Int("HouseAmount",pInfo[playerid][HouseAmount]);
	...
	INI_Int("Hits",pInfo[playerid][sHits]);
...
{
	SetPlayerWantedLevel(playerid, 0);
	if(pInfo[playerid][Class] == cHitman) SetPlayerTeam(playerid, NO_TEAM);
	else SetPlayerTeam(playerid, 1);
